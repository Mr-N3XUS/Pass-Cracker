<div align="center">

# 🔓 Password-Cracker | 👨‍💻 Developed by Mr-B1nary

### *A Lightweight Password Recovery Tool*

<br>

[![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)](https://www.python.org/)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Mr-B1nary)

<br>

---

## 📖 ABOUT

> **Pass-Cracker** is a minimal Python-based password recovery tool designed for educational purposes. It demonstrates basic password cracking techniques in a controlled environment. This project is intended to help understand the importance of strong passwords and security practices.

---

## ⚡ FEATURES

<br>

| 🔍 **Dictionary Attack** | 🐍 **Pure Python** | 📚 **Educational Tool** |
|:-----------------------:|:------------------:|:-----------------------:|
| Attempts to match passwords using a wordlist | No external dependencies | Learn about password security |

| 💻 **Command-Line Interface** | ⚡ **Fast Execution** | 🔒 **Controlled Environment** |
|:----------------------------:|:--------------------:|:------------------------------:|
| Simple and straightforward usage | Optimized for short passwords | Limited to 6 characters for safety |

---

## 🛠️ TECH STACK

<br>

| Technology | Role |
|:----------:|:----:|
| **Python 3** | Core logic & execution |

---
### 🚀 How to Run
```bash
git clone https://github.com/Mr-B1nary/Pass-Cracker.git
```
```bash
cd Password-Cracker
```
```bash
python tamrin.py
```
---
<h3 align="center">Connect with me:</h3>
<p align="center">
<a href="https://instagram.com/mr_b1nary" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/instagram.svg" alt="mr_b1nary" height="30" width="40" /></a>
<a href="https://t.me/Mr_B1naryy" target="blank"><img align="center" src="https://upload.wikimedia.org/wikipedia/commons/8/82/Telegram_logo.svg" alt="mr_b1nary" height="30" width="40" /></a>
</p>

⭐ If you like this project, give it a star⭐<br>
[![GitHub stars](https://img.shields.io/github/stars/Mr-B1nary/Timer?style=for-the-badge&logo=github&color=yellow)](https://github.com/Mr-B1nary/Timer/stargazers)
